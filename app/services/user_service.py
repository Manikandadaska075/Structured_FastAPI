...
class UserService:
    ...
    def register_admin(session, user_data: dict):
        logger.info(f"Admin registration start for: {user_data['email']}")

        if user_data["designation"].lower() != "hr":
            logger.warning("Non-HR tried to register as admin")
        ...
        logger.info(f"Admin created successfully: {user.email}")
        return UserRepository.create_user(session, user)

    @staticmethod
    def login_user(session, email: str, password: str):
        logger.info(f"Login process started for: {email}")
        user = UserRepository.get_user_by_email(session, email)
        if not user:
    ...
    def employee_creation(creation, current_user, session):
        logger.info(f"Employee creation requested by admin: {current_user.email}")
        if not current_user.isSuperUser:
            logger.error("Non-admin tried to create employee")
        ...
        if existing_user:
            logger.warning(f"Employee email already exists: {creation['email']}")
        ...
        logger.info(f"Employee created: {employee.email}")
        return UserRepository.employee_details_add(session, employee)
    
    @staticmethod
    def user_update(data:dict, session, admin):        
        if "password" in data:
    ...
    def employee_update(data:dict, employeeEmail:Optional[str], current_user:User, session):
        if current_user.isSuperUser:
            if not employeeEmail:
                logger.warning("Admin update missing employee email")
        ...
        if "password" in data:
            logger.debug("Employee password update detected")
            data["password"] = UserService.hash_password(data["password"])

        updated = UserRepository.user_update(session, employee, data)
        logger.info(f"Employee updated successfully: {updated.email}")
        return updated
    ...
    def user_deletion(session, admin, adminOrEmployeeEmail: str):
        ...
        logger.warning(f"Delete request for: {adminOrEmployeeEmail}")
        record = UserRepository.get_user_by_email(session, adminOrEmployeeEmail)
        if not record:
            logger.error("Deletion target not found")
    ...
    @staticmethod
    def logout_user():
        logger.info("Running logout cleanup")
        now = datetime.now()
